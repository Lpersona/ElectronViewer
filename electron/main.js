...
const { getServiceUrl, checkFileFromat, createService, createEarthService } = require("./function");

const { ServerCollection } = require("./ServerCollection");
...
ipcMain.on("getFiles", (event) => {
	...
	promise.then((res) => {
		if (res.filePaths && res.filePaths.length !== 0) {
			...
			const { service_url, file_name } = getServiceUrl(file_path);
			const is_json = checkFileFromat(file_path, "json");

			if (is_json) {
				createService(service_url).then((res) => {
					const { port_number, tile_server } = res;
...
// 加载地图
ipcMain.on("addLayer", (event, name, serviceUrl, type) => {
	store.set(name, { name, type, serviceUrl });
});

// 删除地图
ipcMain.on("removeLayer", (event, name) => {
	store.delete(name);
});

// 读取底图信息
ipcMain.on("loadLayer", (event) => {
	event.reply("loadLayerResponse", store.store);
});
